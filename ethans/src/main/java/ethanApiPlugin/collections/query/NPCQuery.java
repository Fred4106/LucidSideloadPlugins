...
public class NPCQuery {
    ...
    public NPCQuery sorted(Function<NPC,Integer> sortFunc, int max) {
        npcs = npcs.stream().sorted(Comparator.comparingInt(sortFunc::apply)).collect(Collectors.toList());
        npcs = npcs.subList(0, Math.min(npcs.size(), max));
        return this;
    }
    ...
    public List<NPC> results() {
        return ImmutableList.copyOf(npcs.stream().sorted(Comparator.<NPC>comparingInt(n -> {
            return n.getWorldLocation().distanceTo2D(client.getLocalPlayer().getWorldLocation());
        })).collect(Collectors.toList()));
    }
}
