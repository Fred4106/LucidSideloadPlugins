...
object KroovyTokenStyle {
	def apply(token: Token): KroovyTokenStyle = {
		Option(token).map(t => new KroovyTokenStyle(t.getType)).getOrElse(new KroovyTokenStyle(0))
	}
	...
	def getStyle(token: Token): Option[String] = {
		getStyle(token.getType)
	}
	def getStyle(i: Int): Option[String] = {
		val x = Option(i match {
...
class KroovyTokenStyle(val tokenType: Int) extends StyleInf {
	def toStyle: String = {
		KroovyTokenStyle.getStyle(tokenType).getOrElse("text")
	}

	def getTokenTypeName: String = {
		KroovyTokenStyle.tokenTypeToName.getOrElse(tokenType, "Unknown")
	}
}
