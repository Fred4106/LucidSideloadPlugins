...
public class ActionManager
{
	...
	@Inject private FredsTickManager tickManager;
	...
	@Getter private ActionEnum currentAction;

	@Getter @Setter private int currentProductId = -1;

	private static int calculateActionTicks(ActionEnum action, int actionCount)
	{
		int nTicksElapsed = 0;
		...
		int realActionCount = action == ActionEnum.SMITHING_WITH_SMITH_OUTFIT ? (int) (actionCount * .8) : actionCount;
	...
	public void setAction(ActionEnum action, int actionCount, int itemId)
	{
		...
		long duration = calculateActionTicks(action, actionCount) * FredsTickManager.PERFECT_TICK_TIME;
	...
	public long getApproximateCompletionTime()
	{
		...
		return Math.round((ticksLeft * FredsTickManager.PERFECT_TICK_TIME) - timeSinceTick);
	}
