...
public class W_ToxicBlowpipe extends ChargedItemWithStorage {
    public W_ToxicBlowpipe(Provider provider) {
        this(provider, FredsItemChargesConfig.toxic_blowpipe, ItemID.TOXIC_BLOWPIPE_LOADED, new TriggerItem[]{
    ...
    public W_ToxicBlowpipe(Provider provider, String configKey, int itemId, TriggerItem[] items) {
        ...
        this.triggers.addAll(List.of(
            ...
            new OnChatMessage("Darts: None\\. Scales: (?<scales>.+) \\(.*\\).").matcherConsumer(m -> {
                StorageItem scales = new StorageItem(ItemID.SNAKEBOSS_SCALE, FredsItemChargesPlugin.getNumberFromCommaString(m.group("scales")));
            ...
            new OnChatMessage("Darts: (?<dartstype>.+) x (?<dartsamount>.+)\\. Scales: (?<scales>.+) \\(.*\\).").matcherConsumer(m -> {
                storage.clearAndPut(ItemID.SNAKEBOSS_SCALE, FredsItemChargesPlugin.getNumberFromCommaString(m.group("scales")));

                Optional<StorageItem> darts = getStorageItemFromName(m.group("dartstype"), FredsItemChargesPlugin.getNumberFromCommaString(m.group("dartsamount")));
                if (darts.isPresent()) {
                    storage.put(darts);
                }
            }).onItemClick(),
            ...
            new OnScriptPreFired(1651).scriptConsumer((script) -> {
                Optional<Widget> widget = FredsItemChargesPlugin.getWidget(provider.client, 584, 5);
                if (
            ...
            new OnAnimationChanged(5061).isEquipped().consumer(() -> {
                ...
                for (StorageItem item : storage.getStorage().getItems()) {
                    if (item.itemId != ItemID.SNAKEBOSS_SCALE && FredsItemChargesPlugin.guessIfRangedAmmoRetrievalWasSuccessful(provider)) {
    ...
    public String getTotalChargesString() {
        ...
        return FredsItemChargesPlugin.getChargesMinified(charges.orElse(0));
    }
