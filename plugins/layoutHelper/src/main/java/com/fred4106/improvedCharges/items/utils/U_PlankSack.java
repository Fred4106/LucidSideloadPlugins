...
public class U_PlankSack extends ChargedItemWithStorage {
    ...
    public U_PlankSack(final Provider provider) {
        super(com.fred4106.improvedCharges.Constants.PLANK_SACK, ItemId.PLANK_SACK, provider);
...
//            }),
            ...
            // Empty to inventory.
            new OnItemContainerChanged(INVENTORY).emptyStorageToInventory().onMenuOption("Empty", FredsItemChargesPlugin.menuOptionEmptyToInventory),

            // Fill from inventory.
            new OnItemContainerChanged(INVENTORY).fillStorageFromInventory().onMenuOption("Fill", FredsItemChargesPlugin.menuOptionFillFromInventory),
            ...
            new OnScriptPreFired(1632).scriptConsumer(script -> {
                ...
                final Optional<Widget> materialsWidget = FredsItemChargesPlugin.getWidget(provider.client, 458, 3 + nthItemToBuild, 3);
                if (!materialsWidget.isPresent()) return;
