...
public class W_ToxicBlowpipe extends ChargedItemWithStorage {
    public W_ToxicBlowpipe(final Provider provider) {
        this(com.fred4106.improvedCharges.Constants.TOXIC_BLOWPIPE, ItemId.TOXIC_BLOWPIPE, provider);
    ...
    public W_ToxicBlowpipe(final String configKey, final int itemId, final Provider provider) {
        ...
        this.triggers = new TriggerBase[] {
            ...
            new OnChatMessage("Darts: (?<dartstype>.+) x (?<dartsamount>.+)\\. Scales: (?<scales>.+) \\(.*\\).").matcherConsumer(m -> {
                ...
                final Optional<StorageItem> darts = getStorageItemFromName(m.group("dartstype"), FredsItemChargesPlugin.getNumberFromCommaString(m.group("dartsamount")));
                if (darts.isPresent()) {
                    storage.put(darts);
                }
            }),
            ...
            new OnScriptPreFired(1651).scriptConsumer((script) -> {
                final Optional<Widget> widget = FredsItemChargesPlugin.getWidget(provider.client, 584, 5);
                if (
                    widget.isPresent() &&
