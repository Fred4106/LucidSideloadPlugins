...
public abstract class BaseMap implements IMap
{
	...
	protected FredsTeleportMapsConfig config;
	protected Client client;
	protected ClientThread clientThread;
	final protected List<UITeleport> activeUITeleports;
	private boolean active;

	public BaseMap(FredsTeleportMapsPlugin plugin, FredsTeleportMapsConfig config, Client client, ClientThread clientThread, boolean active)
	{
		this.plugin = plugin;
		this.config = config;
		this.client = client;
	...
	protected UIHotkey createHotKey(Widget container, HotKeyDefinition hotKeyDefinition, String hotKeyLabel)
	{
		...
		boolean displayHotkeys = this.config.displayHotkeys() && !hotKeyLabel.isBlank();
