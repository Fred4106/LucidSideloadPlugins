...
public class ClueCompassMap extends BaseMap implements IAdventureMap
{
	...
	{
		super(plugin, config, client, clientThread, config.showClueCompassMap());
	...
	public void onConfigChanged(ConfigChanged e)
	{
		switch (e.getKey())
		{
			case TeleportMapsConfig.KEY_SHOW_CLUECOMPASS_MAP:
				this.setActive(config.showClueCompassMap());
			case TeleportMapsConfig.KEY_SHOW_CLUECOMPASS_MAP_HOTKEYS:
			case TeleportMapsConfig.KEY_DISPLAY_HOTKEYS:
				this.updateTeleports((teleport) -> {
					teleport.setHotKeyVisibility(config.displayHotkeys() && config.showClueCompassMapHotkeys());
				});
			default:
