package com.fredplugins.scurriushelper.helpers

import com.fredplugins.common.utils.SInteractionUtils
import com.fredplugins.scurriushelper.{BossToolTrait, FredsScurriusHelperConfig}
...
class ScurriusLogic(using client: Client, config: FredsScurriusHelperConfig) extends BossToolTrait {
	...
	private def onGraphicsObjectCreated(event: GraphicsObjectCreated): Unit = {
		...
		if (id == FALLING_CEILING_GRAPHIC) fallingCeilingToTicks = fallingCeilingToTicks.updated(graphicsObject, DURATION)
	}
...
//				.build,
	...
	def localPlayerWorldPoint: WorldPoint = Try(WorldPoint.fromLocalInstance(client, client.getLocalPlayer.getLocalLocation)).get
	def getRegion: Int = Try(WorldPoint.fromLocalInstance(client, client.getLocalPlayer.getLocalLocation).getRegionID).getOrElse(-1)
	def getRegionX: Int = Try(WorldPoint.fromLocalInstance(client, client.getLocalPlayer.getLocalLocation).getRegionX).getOrElse(-1)
	def getRegionY: Int = Try(WorldPoint.fromLocalInstance(client, client.getLocalPlayer.getLocalLocation).getRegionY).getOrElse(-1)

	override def inArea(): Boolean = {
		val toRet = localPlayerWorldPoint.pipe(x => x.getRegionID == 13210 && x.getRegionX >= 23)
		toRet
	}
}
