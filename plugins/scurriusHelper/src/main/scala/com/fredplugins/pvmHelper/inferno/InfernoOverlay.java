...
import com.fredplugins.pvmHelper.inferno.displaymodes.InfernoPrayerDisplayMode;
import com.fredplugins.pvmHelper.inferno.displaymodes.InfernoSafespotDisplayMode;
...
public class InfernoOverlay extends Overlay
{
	...
	private final FredsInfernoPlugin plugin;
	private final FredsInfernoConfig config;
	private final Client client;

	@Inject
	private InfernoOverlay(final Client client, final FredsInfernoPlugin plugin, final FredsInfernoConfig config)
	{
		this.client = client;
		this.plugin = plugin;
		this.config = config;
	...
	{
		final Widget meleePrayerWidget = client.getWidget(WidgetInfoExtended.PRAYER_PROTECT_FROM_MELEE.getId());
		final Widget rangePrayerWidget = client.getWidget(WidgetInfoExtended.PRAYER_PROTECT_FROM_MISSILES.getId());
		final Widget magicPrayerWidget = client.getWidget(WidgetInfoExtended.PRAYER_PROTECT_FROM_MAGIC.getId());
	...
	private void renderDescendingBox(Graphics2D graphics, InfernoNPC.Attack currentAttack, InfernoNPC.Attack priorityAttack, int ticksUntilAttack)
	{
		...
		final Widget prayerWidget = client.getWidget(PrayerExtended.getWidgetId(currentAttack.getPrayer()));
	...
	private Color getCurrentAttackDescendingBarColor(InfernoNPC.Attack currentAttack, InfernoNPC.Attack priorityAttack, int ticksUntilAttack) {
		...
		if(PrayerInteraction.isActive(priorityAttack.getPrayer()))
		{
	...
	private void renderPrayerIconOverlay(Graphics2D graphics)
	{
		if (plugin.getClosestAttack() != null)
		{
			...
			if (PrayerInteraction.isActive(Prayer.PROTECT_FROM_MAGIC))
			{
				prayerForAttack = InfernoNPC.Attack.MAGIC;
			}
			else if (PrayerInteraction.isActive(Prayer.PROTECT_FROM_MISSILES))
			{
				prayerForAttack = InfernoNPC.Attack.RANGED;
			}
			else if (PrayerInteraction.isActive(Prayer.PROTECT_FROM_MELEE))
			{
			...
			{
				final Widget prayerWidget = client.getWidget(PrayerExtended.getWidgetId(plugin.getClosestAttack().getPrayer()));
