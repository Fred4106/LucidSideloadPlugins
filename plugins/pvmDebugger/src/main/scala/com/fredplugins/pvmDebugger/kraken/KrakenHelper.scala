		...
		if(cooldown > 0) {
			...
			if(krakenNpcOpt.isDefined) {
				...
				if (countdownTillFirstAttack > 0) {
					...
					cooldown = if(InventoryUtils.wieldItem(ItemID.MAGICTRAINING_WAND_MASTER)) 6 else 0
				}
				else if (myHp > config.upperHpThreshold() && EquipmentUtils.getWepSlotItem.getId != ItemID.WARPED_SCEPTRE) {
					cooldown = if(InventoryUtils.wieldItem(ItemID.WARPED_SCEPTRE)) 6 else 0
				}
					...
					} else if(InventoryUtils.contains(ItemID.FISHING_EXPLOSIVE) && localPlayer.getInteracting == null) {
