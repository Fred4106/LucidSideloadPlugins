...
import net.runelite.api.gameval.ItemID.{UNCUT_SAPPHIRE, UNCUT_EMERALD, UNCUT_RUBY, UNCUT_DIAMOND, UNCUT_DRAGONSTONE, UNCUT_OPAL, UNCUT_JADE, UNCUT_RED_TOPAZ, CHISEL, SUN_KISSED_BONE, ARCEUUS_ESSENCE_BLOCK_DARK}
	...
	def onWidgetItemMenuEntryAdded(menuOptionAdded: MenuEntryAdded): Unit = {
		...
		if (me.getType == MenuAction.WIDGET_TARGET && me.getParam1 == InterfaceID.Inventory.ITEMS) {
			...
			val b = a.map(_.id).collect {
				...
				case SUN_KISSED_BONE => List(CHISEL)
				case BLESSED_DRAGON_BONES_SUPERIOR => List(CHISEL)
