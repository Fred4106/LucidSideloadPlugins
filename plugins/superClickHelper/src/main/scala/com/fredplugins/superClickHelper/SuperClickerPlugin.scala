...
import net.runelite.api.gameval.ItemID.{GLASSBLOWINGPIPE, MOLTEN_GLASS}
...
import net.runelite.api.gameval.ItemID.{UNCUT_SAPPHIRE, UNCUT_EMERALD, UNCUT_RUBY, UNCUT_DIAMOND, UNCUT_DRAGONSTONE, UNCUT_OPAL, UNCUT_JADE, UNCUT_RED_TOPAZ, JEWELLERS_CHISEL, CHISEL, SUN_KISSED_BONE, ARCEUUS_ESSENCE_BLOCK_DARK}
import net.runelite.api.gameval.ItemID.{IRON_ORE, COAL, MITHRIL_ORE, ADAMANTITE_ORE, RUNITE_ORE}
import net.runelite.api.gameval.ItemID.{JUG_WINE, SUNFIRESPLINTER, SUNFIRESPLINTER_2, SUNFIRESPLINTER_3, SUNFIRESPLINTER_4, BLESSED_DRAGON_BONES_SUPERIOR}
...
import net.runelite.api.gameval.ItemID.{WINT_BRUMA_ROOT, LOGS, OAK_LOGS, WILLOW_LOGS, TEAK_LOGS, MAPLE_LOGS, MAHOGANY_LOGS, YEW_LOGS, MAGIC_LOGS, REDWOOD_LOGS}
...
import net.runelite.api.gameval.ItemID.{VIAL_WATER, TOADFLAX, IRIT_LEAF, KWUARM, CADANTINE, DWARF_WEED, LANTADYME, TORSTOL, TOADFLAXVIAL, IRITVIAL, KWUARMVIAL, CADANTINEVIAL, DWARFWEEDVIAL, LANTADYMEVIAL, TORSTOLVIAL}
import net.runelite.api.gameval.ItemID.{VIAL_WATER, TOADFLAX, IRIT_LEAF, KWUARM, CADANTINE, DWARF_WEED, LANTADYME, TORSTOL, TOADFLAXVIAL, IRITVIAL, KWUARMVIAL, CADANTINEVIAL, DWARFWEEDVIAL, LANTADYMEVIAL, TORSTOLVIAL}
import net.runelite.api.gameval.ItemID.{CRUSHED_BIRD_NEST, EYE_OF_NEWT, LIMPWURT_ROOT, WHITE_BERRIES, WINE_OF_ZAMORAK, DRAGON_SCALE_DUST, CACTUS_POTATO, _4DOSE2ATTACK}
	...
	def onWidgetItemMenuEntryAdded(menuOptionAdded: MenuEntryAdded): Unit = {
		...
		if (me.getType == MenuAction.WIDGET_TARGET && me.getParam1 == InterfaceID.Inventory.ITEMS) {
			...
			val b = a.map(_.id).collect {
				...
				case BRUT_SPAWNING_TROUT | BRUT_SPAWNING_SALMON | BRUT_STURGEON  => List(KNIFE,FLETCHING_KNIFE)
				case KNIFE | FLETCHING_KNIFE => List(WINT_BRUMA_ROOT, MAPLE_LOGS, YEW_LOGS, MAGIC_LOGS, BRUT_SPAWNING_TROUT, BRUT_SPAWNING_SALMON, BRUT_STURGEON)
				case TINDERBOX => List(MAPLE_LOGS, YEW_LOGS, MAGIC_LOGS)
				case MAPLE_LOGS | YEW_LOGS | MAGIC_LOGS => List(TINDERBOX, KNIFE, FLETCHING_KNIFE)
				case WINT_BRUMA_ROOT => List(KNIFE, FLETCHING_KNIFE)
				case ZAMORAK_GRAPES => List(JUG_WATER)
				case GRAPES => List(JUG_WATER)
				case JUG_WATER => List(GRAPES, ZAMORAK_GRAPES)
				case VIAL_WATER => List(TOADFLAX, IRIT_LEAF, KWUARM, CADANTINE, DWARF_WEED, LANTADYME, TORSTOL)
				case TOADFLAX | IRIT_LEAF | KWUARM | CADANTINE | DWARF_WEED | LANTADYME | TORSTOL => List(VIAL_WATER)
				case TOADFLAXVIAL => List(CRUSHED_BIRD_NEST)
				case IRITVIAL => List(EYE_OF_NEWT)
				case KWUARMVIAL => List(LIMPWURT_ROOT)
				case CADANTINEVIAL => List(WHITE_BERRIES)
				case DWARFWEEDVIAL => List(WINE_OF_ZAMORAK)
				case LANTADYMEVIAL => List(DRAGON_SCALE_DUST, CACTUS_POTATO)
				case TORSTOLVIAL => List(_4DOSE2ATTACK)
				case JUG_WINE => List(SUNFIRESPLINTER, SUNFIRESPLINTER_2, SUNFIRESPLINTER_3, SUNFIRESPLINTER_4)
				case SUNFIRESPLINTER | SUNFIRESPLINTER_2 | SUNFIRESPLINTER_3 | SUNFIRESPLINTER_4 => List(JUG_WINE)
				case CHISEL | JEWELLERS_CHISEL => List(ARCEUUS_ESSENCE_BLOCK_DARK, UNCUT_SAPPHIRE, UNCUT_EMERALD, UNCUT_RUBY, UNCUT_DIAMOND, UNCUT_DRAGONSTONE, UNCUT_OPAL, UNCUT_JADE, UNCUT_RED_TOPAZ)
				case ARCEUUS_ESSENCE_BLOCK_DARK => List(CHISEL, JEWELLERS_CHISEL)
				case SUN_KISSED_BONE => List(CHISEL, JEWELLERS_CHISEL)
				case BLESSED_DRAGON_BONES_SUPERIOR => List(CHISEL, JEWELLERS_CHISEL)
				case UNCUT_SAPPHIRE => List(CHISEL, JEWELLERS_CHISEL)
				case UNCUT_EMERALD => List(CHISEL, JEWELLERS_CHISEL)
				case UNCUT_RUBY => List(CHISEL, JEWELLERS_CHISEL)
				case UNCUT_DIAMOND => List(CHISEL, JEWELLERS_CHISEL)
				case UNCUT_DRAGONSTONE => List(CHISEL, JEWELLERS_CHISEL)
				case UNCUT_OPAL => List(CHISEL, JEWELLERS_CHISEL)
				case UNCUT_JADE => List(CHISEL, JEWELLERS_CHISEL)
				case UNCUT_RED_TOPAZ => List(CHISEL, JEWELLERS_CHISEL)
				case MOLTEN_GLASS => List(GLASSBLOWINGPIPE)
				case GLASSBLOWINGPIPE => List(MOLTEN_GLASS)
