...
public class IconIndex {
    ...
    public void inc(FredsLootFiltersPlugin plugin, BufferedImageProvider provider, PluginTileItem item, int... height) {
    ...
    public void reset(FredsLootFiltersPlugin plugin) {
        clear();
        for (var entry : plugin.getTileItemIndex().entrySet()) {
            for (var item : entry.getValue()) {
                ...
                if (match != null && match.getIcon() != null) {
                    inc(plugin, match.getIcon(), item, /*match.isCompact()*/plugin.getConfig().compactMode() ? plugin.getConfig().compactRenderSize() : 16);
                }
            }
        }
    }
