...
case class DisplayConfig(
	...
	def isHidden: Boolean = hideAll.getOrElse(false)
	def isShowLootbeam: Boolean = Option.when(!isHidden)(showLootbeam).flatten.exists(_ == true)
	def isShowValue: Boolean = showValue.exists(_ == true)
	...
	def isHideOverlay: Boolean = Option.when(!isHidden)(hideOverlay).flatten.exists(_ == true)
}
...
object DisplayConfig {
	...
	def apply(txtColor: Color): DisplayConfig = {
		empty.builder.withTextColor(txtColor).build
	}
}
