...
public class LootFilterManager {
	...
	public List<LootFilter> loadFilters() {
		...
		var files = Arrays.stream(FredsLootFiltersPlugin.FILTER_DIRECTORY.listFiles())
	...
	public void saveNewFilter(String name, String src) throws IOException {
		...
		var newFile = new File(FredsLootFiltersPlugin.FILTER_DIRECTORY, toFilename(name));
	...
	public void updateFilter(String filename, String src) throws IOException {
		var file = new File(FredsLootFiltersPlugin.FILTER_DIRECTORY, filename);
		if (!file.exists()) {
