...
//		WorldRegion.polygons(worldPoints).appendedAll(
//
//		)

		WorldRegion.polygons(worldPoints.flatMap(wp => wp.getInstanced).distinct)
	}
}

object WorldRegion extends ShimUtils.Logging() {
	...
	private def polygons(worldPoints: Seq[WorldPoint])(using client: Client): Array[Polygon] = {
		log.trace("worldPoints: {}", worldPoints)
		var borderEdges: Seq[((Int, Int), (Int, Int))] = worldPoints.flatMap(_.edges).pipe(alledges => alledges.filter(e => alledges.count(_ == e) == 1))
		log.trace("borderEdges: {}", borderEdges) //borderEdges.mkString("borderEdges: [\n\t", ", \n\t", "\n\t]\n"))
