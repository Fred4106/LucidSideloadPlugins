...
object STimedPotion extends enumeratum.Enum[STimedPotion] {
	private lazy val client: Client = RuneLite.getInjector().getInstance[Client](classOf[Client])
	private lazy val clientThread: ClientThread = RuneLite.getInjector().getInstance[ClientThread](classOf[ClientThread])
