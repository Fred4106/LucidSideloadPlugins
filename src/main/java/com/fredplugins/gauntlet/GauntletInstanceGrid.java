...
public class GauntletInstanceGrid
{
    ...
    @Inject
    public GauntletInstanceGrid(final Client client, final FredGauntletPlugin plugin, final FredGauntletConfig config)
    {
        this.client = client;
        this.plugin = plugin;
        this.config = config;
    ...
    public void reset()
    {
        rooms = new com.fredplugins.gauntlet.GauntletRoom[GRID_SIZE][GRID_SIZE];
        initialized = false;
    }
    ...
    public void initialize()
    {
        ...
        com.fredplugins.gauntlet.GauntletRoom hunllefRoom = getHunllefRoomFromBarriers(barriers);
        ...
        com.fredplugins.gauntlet.GauntletRoom startingRoom = new com.fredplugins.gauntlet.GauntletRoom(startMinX, startMinY);
        ...
        for (int x = 0; x < GRID_SIZE; x++)
        {
            for (int y = 0; y < GRID_SIZE; y++)
            {
                if (getRoom(x, y) == null)
                {
                    rooms[x][y] = new com.fredplugins.gauntlet.GauntletRoom(basePoint.getX() + (x * ROOM_SIZE), basePoint.getY() - (y * ROOM_SIZE));
                }
            }
        }
        initialized = true;
    ...
    public com.fredplugins.gauntlet.GauntletRoom getNextUnlitRoomFirstPass()
    {
        for (Point p : beginningSearchPath)
        {
            com.fredplugins.gauntlet.GauntletRoom room = getRoom(p.getX(), p.getY());
    ...
    public com.fredplugins.gauntlet.GauntletRoom getNextUnlitRoomSecondPass()
    {
    ...
    private com.fredplugins.gauntlet.GauntletRoom getHunllefRoomFromBarriers(List<TileObject> barriers)
    {
        int minX = -1;
        int maxY = -1;
        ...
        return new com.fredplugins.gauntlet.GauntletRoom(minX - 1, maxY + 1);
    }
    ...
    private Point getStartingRoomGridLocation(TileObject toolStorage)
    {
        ...
        com.fredplugins.gauntlet.GauntletRoom hunllefRoom = getRoom(3, 3);
