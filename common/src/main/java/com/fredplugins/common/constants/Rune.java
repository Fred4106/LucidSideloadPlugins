...
public enum Rune {
	...
	private static final Map<Integer, Rune> lookupMap;

	static {
		ImmutableMap.Builder<Integer, Rune> b = new ImmutableMap.Builder<Integer, Rune>();
		Arrays.stream(values()).forEach(r -> {
			b.put(r.varbitValue, r);
		});
		lookupMap = b.build();
	}
	public static Optional<Rune> getRuneFromIndex(int index) {
		return  Optional.ofNullable(lookupMap.get(index));
	}
	public static Optional<Rune> getRuneFromItemId(int itemId) {
